...
public class Main_EX2_PM_3_1_D extends JFrame implements KeyListener {
    ...
    final int width = 300, height = 300, boxSize = 7;
    static int direction = 1;
    final int LEFT = 1, RIGHT = 2, TOP = 3, BOTTOM = 4;
    Point dotPoint = new Point();
    int score = 0;
    final long timeLimit = (5 * 60 * 1000);
    final int scoreLimit = 2;
    long startTime;


    public Main_EX2_PM_3_1_D() {
        System.out.println("Maximum Game Time: " + (timeLimit/60/1000) +" (minutes)" + ", Scores to Win: " + scoreLimit);
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (timeLimit <= (currentTime - startTime)) {
            System.out.println("Sorry!...Time Finished.");
            System.exit(-11);
        }

        if (score == scoreLimit) {
            System.out.println("Congratulation!...Full Score achieved.");
            System.exit(11);
        }
    ...
    private void handleCrossBorder() {
        if (pacmanPoint.x < 7 / boxSize) {
            pacmanPoint.x = 7 / boxSize;
        } else if (pacmanPoint.x > 287 / boxSize) {
            pacmanPoint.x = 287 / boxSize;
        }

        if (pacmanPoint.y < 28 / boxSize) {
            pacmanPoint.y = 28 / boxSize;
        } else if (pacmanPoint.y > 287 / boxSize) {
            pacmanPoint.y = 287 / boxSize;
        }
    }
